...
objetivo): objeto_destino(objetivo), pos_destino(objetivo->obtener_esquina()) {
...
*unidad, Mapa &mapa, int tiempo_transcurrido) {
	// "MovimientoParaAtacar::actualizar "<< pos_destino.first << '-' << pos_destino.second << std::endl;
	...
	if (mapa.de_pixel_a_baldosa(pos_destino) != 
		mapa.de_pixel_a_baldosa(objeto_destino->obtener_esquina())) {
		unidad->asignar_nuevo_camino(mapa.
		obtener_camino(unidad->obtener_centro(),
			objeto_destino->obtener_esquina(),unidad));
		pos_destino = objeto_destino->obtener_esquina();
	}

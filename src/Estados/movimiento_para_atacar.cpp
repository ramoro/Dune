...
*unidad, Mapa &mapa, int tiempo_transcurrido) {
	...
	if (mapa.de_pixel_a_baldosa(pos_destino) != mapa.de_pixel_a_baldosa(objeto_destino->obtener_centro())) {
		unidad->asignar_nuevo_camino(mapa.
		obtener_camino(unidad->obtener_centro(),objeto_destino->obtener_centro()));

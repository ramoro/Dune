...
centro, Root &root) : 
	UnidadInfanteria(root["InfanteriaLigera"].get("rango", 0).asInt(),
	 ...
	 root["InfanteriaLigera"].get("dimension_alto", 0).asInt(), centro) {
		...
		for (unsigned int i = 0; i < root["InfanteriaLigera"]["edificios_necesarios"].size(); i++) {
			ids_tipos_edificios_necesarios.push_back(root["InfanteriaLigera"]["edificios_necesarios"][i].asInt());
		}
	}
